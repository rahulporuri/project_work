...
plt.cla()
plt.hold(True)
plt.semilogy(numpy.linspace(Nics,Nshss,len(hk_array[:4850])), numpy.absolute(hk_array[:4850]))
plt.savefig('abs_hk_vs_N_k3.png')

plt.cla()
plt.hold(True)
plt.plot(numpy.linspace(Nics,Nshss,len(hk_array[:4850])), hk_array.imag[:4850])
