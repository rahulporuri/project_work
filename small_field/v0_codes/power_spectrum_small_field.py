...
tps_file = open('power_spectrum_py.txt','w')
phi_file = open('phi_vs_N_py.txt','w')
H_file = open('H_vs_N_py.txt','w')
DH_file = open('DH_vs_N_py.txt', 'w')
eps_file = open('eps1_vs_N_py.txt','w')
...
for N in N_array:
	H_file.write(str(N)+"\t"+str(H(N))+"\n")
	DH_file.write(str(N)+"\t"+str(DH(N))+"\n")
'''
plt.cla()
plt.hold(True)
plt.xlim([Ni,Nf])
