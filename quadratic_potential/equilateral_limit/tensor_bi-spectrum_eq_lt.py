...
V = lambda phi : m**2.*phi**2./2.
dV = lambda phi : m**2.*phi
...
while N < Nf+step:
	try : 
		...
		print N, phi_, Dphi_, V(phi_), dV(phi_)
		break 
...
numpy.savetxt('N_eq_lt.dat', N_array)
numpy.savetxt('phi_eq_lt.dat', phi_array)
numpy.savetxt('H_eq_lt.dat', numpy.asarray([H(N) for N in N_array]))
numpy.savetxt('DH_eq_lt.dat', numpy.asarray([DH(N) for N in N_array]))

plt.cla()
plt.xlabel(r'$\phi$')
plt.ylabel(r'$V(\phi)$')
plt.title(r'$V(\phi)$ as a function of $\phi$')
...
plt.cla()
plt.xlabel(r'$N$')
plt.ylabel(r'${\rm d}\phi/{\rm d}N$')
plt.title(r'${\rm d}\phi/{\rm d}N$ as a function of $N$')
python, = plt.plot(N_array, Dphi_array, '--')
plt.legend([python],['python'])
plt.savefig('Dphi_vs_N.png')
...
python, = plt.plot(N_array, numpy.asarray([Dphi(N)**2./2 for N in N_array]), '--')
plt.legend([python],['python'])
...
numpy.savetxt('equilateral_limit.dat', data)
